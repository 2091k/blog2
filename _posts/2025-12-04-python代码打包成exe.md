...
```
pyinstaller --onedir --windowed --name MCP-Manager --collect-all flet --collect-all flet_desktop mcp-server.py
```

#### 参数说明
- --onedir：打包成文件夹（稳定性远好于单文件，flet 推荐这个）
- --windowed：不弹出黑控制台（GUI 程序用；调试闪退请删掉这个）
- --name MCP-Manager：exe 程序名称
- --collect-all flet 完整收集 flet 全部资源
- --collect-all flet_desktop 收集桌面运行依赖
